...
if (!empty($_GET['tag'])) {
    $see_moar = "http://www.".$sitedomain."/results.php?search=".$_GET['tag'];
} else if (!empty($_GET['user'])) {
  $see_moar = "http://www.".$sitedomain."/profile_videos?user=".$_GET['user'];  
}
if($singular != 'true') {
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
		<td> <?php if ($singular != 'true') { echo 'My Videos'; }else{ echo "YuoToob";} ?> // <span style="text-transform: capitalize;"><?php echo !empty($_GET['tag']) ? htmlspecialchars($_GET['tag']) : (!empty($_GET['user']) ? htmlspecialchars($_GET['user']) : (!empty($_GET['id']) ? htmlspecialchars($video['title']) : "")); ?>
		...
		<a href="http://www.<?php echo $sitedomain; ?>" target="_blank"><img src="img/logo_sm.gif" width="38" height="15" border="0"></a>
	</tr>
</table>
</div>
