...
if(isset($_POST['send_button'])) {
    ...
    if (substr($_POST['field_send_email_to'], -strlen($_SERVER['SERVER_NAME'])) === $_SERVER['SERVER_NAME']) {
    ...
    if(!empty($_GET['next'])) {
    $construct_url = "http://www.".$sitedomain."/confirm_email?cid=".$confirmation_email."&next=".htmlspecialchars($_GET['next']);
    } else {
    $construct_url = "http://www.".$sitedomain."/confirm_email?cid=".$confirmation_email;    
    }
    ...
    $mail->Subject = 'YuoToob email confirmation';
    $mail->Body    = 'Hi '.htmlspecialchars($session['username']).',<p>Please click on the following link to confirm your email: <a href="'.$construct_url.'">'.$construct_url.'</a><p>See you back on YuoToob!<p>The YuoToob Team';
...
?>
<div class="pageTitle">Please Confirm Your Email</div>
